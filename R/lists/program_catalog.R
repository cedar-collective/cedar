# program_catalog.R
#
# Degree programs, pre-majors, and variant program codes for UNM units.
# Links program codes from academic_studies to college/dept/degree level.
#
# Columns:
#   college_code  — Banner 2-letter college code (home dept's college, not 'GP' for grad students)
#   dept_code     — Links to unit_catalog$dept_code (and therefore to subject codes)
#   program_code  — The program identifier (middle segment of Banner program code)
#   degree_abbr   — Degree type abbreviation (BA, BS, MA, PHD, GCERT, etc.)
#   degree_level  — Undergraduate | Graduate | Professional | Associate | Certificate
#   program_type  — degree | pre_major | variant
#   canonical_code — For variants/pre-majors: the canonical program code they map to (NA for degree)
#
# program_type values:
#   degree      — actual awarded degree program
#   pre_major   — pre-major / declared-intent status (F-prefix codes); not a degree
#   variant     — extended-campus or alternate-code version of a canonical program (X-prefix codes)
#
# Notes:
#   - MSST (Museum Studies) has an undergrad Certificate and grad MA/MS, but no BA/BS degree.
#     Students can concentrate in MSST from within another major. No 'concentration' row is
#     needed here; the CERT-MSST-AS row captures the formal undergrad program presence.
#   - HMHV (Health, Medicine & Human Values) has a BA but is interdisciplinary (housed in AS).
...
#   - 'variant' rows with canonical_code allow historical program code tracing without losing data.
...
program_catalog <- tibble::tribble(
  ~college_code, ~dept_code, ~program_code, ~degree_abbr, ~degree_level, ~program_type, ~canonical_code,
