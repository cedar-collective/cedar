#' Course Neighbors: Course Adjacency Analysis
#'
#' Analyzes a course's neighboring courses — what students take before, after,
#' and concurrently with a given course:
...
#' results <- get_course_neighbors(cedar_students, opt)
#' }
#'
...
where_to <- function (students, opt) {
  ...
  message("[course-neighbors.R] === WHERE_TO Analysis Starting ===")
  ...
  message("[course-neighbors.R] Across all campuses and registration types (including drops), found ", nrow(student_list), " students in ", target_course)
  ...
  message("[course-neighbors.R] Term-by-term enrollment in ", target_course, ":")
  for(i in 1:min(10, nrow(source_by_term))) {
    message("[course-neighbors.R]   ", source_by_term$target_term[i], " (", 
  ...
  message("[course-neighbors.R] Source term type distribution:")
  for(i in 1:nrow(term_dist)) {
    avg_enrl <- source_by_term %>%
      ...
      pull(avg)
    message("[course-neighbors.R]   ", toupper(term_dist$source_term_type[i]), ": ", 
            term_dist$n[i], " total enrollments (~",
            round(avg_enrl, 1), " avg/term)")
  }
  
  # Map to next terms
  message("[course-neighbors.R] Adding next term column...")
  student_list <- student_list %>% add_next_term_col("target_term", summer = incl_summer)
  
  mapped <- student_list %>% filter(!is.na(next_term))
  message("[course-neighbors.R] Successfully mapped ", nrow(mapped), " students to next terms.")
  ...
  message("[course-neighbors.R] Found ", nrow(disp_merge), " enrollments in next terms.")
  message("[course-neighbors.R] (including ", nrow(disp_merge %>% filter(subject_course == target_course)), " repeat enrollments in ", target_course, ")")
  message("[course-neighbors.R] Number of destination courses: ", length(unique(disp_merge$subject_course)))
  ...
  if(nrow(disp_merge) > 0) {
    ...
    message("[course-neighbors.R] Flow patterns by term type:")
    for(i in 1:nrow(flow_patterns)) {
      message("[course-neighbors.R]   ", toupper(flow_patterns$source_term_type[i]), " → ", 
              toupper(flow_patterns$dest_term_type[i]))
    }
  }
  
  # Calculate by term - no classification grouping (CEDAR naming)
  message("[course-neighbors.R] Calculating term-specific flows...")
  by_term <- disp_merge %>%
    group_by(campus, college, subject_course,
             source_term_type, dest_term_type, next_term) %>%
    summarize(enrolled = n(), .groups = "drop")

  message("[course-neighbors.R] Created ", nrow(by_term), " term-specific records")
  ...
  if(nrow(by_term) > 0) {
    ...
    message("[course-neighbors.R] Top term-specific destination flows (source → target):")
    for(i in 1:nrow(top_terms)) {
      message("[course-neighbors.R]   ", top_terms$next_term[i], ": ",
  ...
  message("[course-neighbors.R] Averaging across terms...")
  ...
  message("[course-neighbors.R] Final: ", nrow(dispersal_avgs), " destination flows.")
  message("[course-neighbors.R] distinct courses: ", length(unique(dispersal_avgs$subject_course)))
  message("[course-neighbors.R] across campuses: ", length(unique(dispersal_avgs$campus)))
  message("[course-neighbors.R] distinct courses at other campuses: ",
  ...
  if(nrow(dispersal_avgs) > 0) {
    ...
    message("[course-neighbors.R] *** Top destination courses ***")
    for(i in 1:nrow(top_dest)) {
      message("[course-neighbors.R]   ", top_dest$subject_course[i], " (",
              toupper(top_dest$source_term_type[i]), " → ",
              toupper(top_dest$dest_term_type[i]), "):")
      message("[course-neighbors.R]     Average: ", round(top_dest$avg_contrib[i], 1),
              " students/term")
      message("[course-neighbors.R]     Range: ", top_dest$min_contrib[i], " to ",
              top_dest$max_contrib[i], " students")
      message("[course-neighbors.R]     Across: ", top_dest$num_terms[i], " terms")
      message("[course-neighbors.R]     Total: ", top_dest$total_students[i],
    ...
    message("[course-neighbors.R] *** Flow direction summary ***")
    for(i in 1:nrow(flow_summary)) {
      message("[course-neighbors.R]   ", toupper(flow_summary$source_term_type[i]), " → ", 
              toupper(flow_summary$dest_term_type[i]), ":")
      message("[course-neighbors.R]     ", flow_summary$courses[i], " destination courses")
      message("[course-neighbors.R]     ", round(flow_summary$avg_total[i], 1), 
              " combined avg students/term")
      message("[course-neighbors.R]     ", flow_summary$students_total[i], 
              " total students across all terms")
    }
  }
  
  message("[course-neighbors.R] === WHERE_TO Complete ===")
...
where_from <- function (students, opt) {
  ...
  message("[course-neighbors.R] === WHERE_FROM Analysis Starting ===")
  ...
  message("[course-neighbors.R] Across all campuses and registration types (including drops), found ", nrow(target_student_list), " students in ", target_course)
  ...
  for(i in 1:min(10, nrow(target_by_term))) {
    message("[course-neighbors.R]   ", target_by_term$target_term[i], " (", 
  ...
  message("[course-neighbors.R] Target term type distribution:")
  for(i in 1:nrow(term_dist)) {
    avg_enrl <- target_by_term %>%
      filter(target_term_type == term_dist$target_term_type[i]) %>%
      summarize(avg = mean(enrl)) %>%
      pull(avg)
    message("[course-neighbors.R]   ", toupper(term_dist$target_term_type[i]), ": ", 
            term_dist$n[i], " total enrollments (~",
            round(avg_enrl, 1), " avg/term)")
  }
  
  # Map to previous terms
  message("[course-neighbors.R] Adding previous term column...")
  ...
  message("[course-neighbors.R] Successfully mapped ", nrow(mapped), " students to previous terms.")
  ...
  message("[course-neighbors.R] (including ", nrow(conduit_students %>% filter(subject_course == target_course)), " repeat enrollments in ", target_course, ")")
  message("[course-neighbors.R] Number of courses contributing: ", length(unique(conduit_students$subject_course)))
  ...
  if(nrow(conduit_students) > 0) {
    ...
    message("[course-neighbors.R] Flow patterns by term type:")
    for(i in 1:nrow(flow_patterns)) {
      message("[course-neighbors.R]   ", toupper(flow_patterns$term_type[i]), " → ", 
              toupper(flow_patterns$target_term_type[i]))
    }
  }
  
  # Calculate by term first (CEDAR naming)
  message("[course-neighbors.R] Calculating term-specific contributions...")
  ...
  if(nrow(by_term) > 0) {
    ...
    message("[course-neighbors.R] Top term-specific feeder flows (source → target):")
    for(i in 1:nrow(top_terms)) {
      message("[course-neighbors.R]   ", top_terms$target_term[i], ": ",
              top_terms$subject_course[i], " (",
  ...
  message("[course-neighbors.R] Final: ", nrow(feeder_avgs), " feeder flows.")
  message("[course-neighbors.R] distinct courses: ", length(unique(feeder_avgs$subject_course)))
  message("[course-neighbors.R] across campuses: ", length(unique(feeder_avgs$campus)))
  message("[course-neighbors.R] distinct courses at other campuses: ",
  ...
  if(nrow(feeder_avgs) > 0) {
    ...
    message("[course-neighbors.R] *** Top feeder courses ***")
    for(i in 1:nrow(top_feeders)) {
      message("[course-neighbors.R]   ", top_feeders$subject_course[i], " (",
              toupper(top_feeders$source_term_type[i]), " → ",
              toupper(top_feeders$target_term_type[i]), "):")
      message("[course-neighbors.R]     Average: ", round(top_feeders$avg_contrib[i], 1),
              " students/term")
      message("[course-neighbors.R]     Range: ", top_feeders$min_contrib[i], " to ",
              top_feeders$max_contrib[i], " students")
      message("[course-neighbors.R]     Across: ", top_feeders$num_terms[i], " terms")
      message("[course-neighbors.R]     Total: ", top_feeders$total_students[i],
              " students (all terms combined)")
    }
    ...
    for(i in 1:nrow(flow_summary)) {
      ...
      message("[course-neighbors.R]     ", flow_summary$courses[i], " feeder courses")
      message("[course-neighbors.R]     ", round(flow_summary$avg_total[i], 1), 
              " combined avg students/term")
      message("[course-neighbors.R]     ", flow_summary$students_total[i], 
              " total students across all terms")
  ...
  message("[course-neighbors.R] === WHERE_FROM Complete ===")
  return(feeder_avgs)
}
...
plot_course_sankey_by_term_with_flow_counts <- function(to_courses, from_courses, opt) {
  ...
  for (current_target_type in target_term_types) {
    ...
    if(length(courses_in_both) > 0) {
      message("[course-neighbors.R] Found ", length(courses_in_both), " courses appearing in both directions: ", 
...
get_course_neighbors <- function(students, opt) {
  ...
  neighbors_summary[["where_to"]] <- where_to_summary
  neighbors_summary[["where_from"]] <- where_from_summary
  neighbors_summary[["where_at"]] <- where_at_summary

  return(neighbors_summary)
}  
