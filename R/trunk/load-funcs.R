...
load_funcs <- function(cedar_base_dir) {
  ...
  # program_map.qs is generated by scripts/build-program-map.R from academic_studies.
  # Assigned to .GlobalEnv so catalog_lookups.R (sourced in global env) can find it.
  # If missing, warn and assign an empty tibble — catalog_lookups.R will produce
  # empty lookup vectors, which is recoverable (dept lookups fall back to subj_to_dept).
  # Re-run scripts/build-program-map.R to regenerate.
  .pm_path <- file.path(cedar_base_dir, "data", "program_map.qs")
  if (file.exists(.pm_path)) {
    program_map <- qs::qread(.pm_path)
  } else {
    warning("[load-funcs.R] program_map.qs not found at: ", .pm_path,
            "\n  Run transform_to_cedar() to generate it (auto-built from academic_studies).",
            "\n  Lookup vectors major_college_to_dept and major_to_dept will be empty.")
    program_map <- tibble::tibble(
      program_code = character(), college_code = character(), dept_code = character(),
      major_code   = character(), degree_abbr  = character(), degree_level = character(),
      program_type = character(), canonical_code = character()
    )
  }
