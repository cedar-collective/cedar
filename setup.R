...
config_lines <- c(
  ...
  "cedar_regstats_thresholds[[\"min_sat_terms\"]] <- 3",
...
if (exists("cedar_data_dir") && dir.exists(cedar_data_dir)) {
  ...
  if (length(data_files) == 0) {
    ...
    cat("  2. Run data parsers: Rscript R/data-parsers/parse-data.R\n")
...
cat("\n")
cat("1️⃣  Web Interface (Shiny App):\n")
cat("   R -e \"shiny::runApp(port = 3838)\"\n")
cat("   Then open: http://localhost:3838\n")
cat("\n")
cat("2️⃣  Interactive R Session (for analysts):\n")
cat("   Open cedar.Rproj in RStudio — the CEDAR tables (cedar_sections,\n")
cat("   cedar_students, ...) and analysis functions load automatically,\n")
cat("   ready to call directly.\n")
cat("\n")
cat("📚 Documentation:\n")
